...
def heron_var(ctx, N, C, H, W, in_dtype, out_dtype):
    ...
    Diff = tvm.te.compute(
        [N, C, H, W],
        lambda n, c, h, w: (In[n, c, h, w] - Mean[n, c, h, w]) * (In[n, c, h, w] - Mean[n, c, h, w]),
        name="Diff",
        tag="injective,Diff"
    )

    A = tvm.te.compute(
        [C, N * H * W], lambda i, j: Diff[j // (H * W), i, j % (H * W) // W, j % W], name="A", tag="injective,A"
    )
