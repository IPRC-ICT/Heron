...
ops = ["gemm", "c1d", "c2d", "c3d", "t2d", "dil", "bmm", "gemv"]
...
def get_onednn_perfs():
    res = []
    for op in ops:
        opname = "onednn/logs/" + op + ".log"
        temp = []
        with open(opname, 'rb') as f:
            for line in f.readlines():
                line = str(line.strip())
                if "avg(ms):" not in line:
                    continue
                perf = float(line.split("avg(ms):")[1][:-2])
                temp.append(perf)
        res.append(temp)
    return res
...
if __name__ == "__main__":
    ...
    onednn = get_onednn_perfs()
    heron_relative = [1]*len(heron)
    pytorch_relative = [geo_mean(np.array(x1) / np.array(x2)) for x1, x2 in zip(heron, onednn)] 
    ...
    plt.bar(x, pytorch_relative, width =width, label='OneDNN', fc='y')
