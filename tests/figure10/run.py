...
if __name__ == '__main__':
    ...
    for i in range(args.start_id, min(args.end_id + 1, len(config.cases))):
        ...
        op_name, case, times = config.cases[case_keys[i]]
        print(case)
        env = Env(measure_option, config)
        latency = run(op_name, case_keys[i], case + [config.in_dtype, config.out_dtype], env)
        cases.append([case, times, latency])
    estimated = 0
    for tup in cases:
        print("Case %s, Times : %d, latency %f ms."%(str(tup[0]), tup[1], tup[2]))
        estimated += tup[2]
    print("Estimated network latency : ", estimated)
